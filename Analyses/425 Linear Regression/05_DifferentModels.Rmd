...
#  ## Simulating Data from a Regression Model
  ...
  set.seed(123) #Allows us to always get the same "random" sample
                #Change to a new number to get a new sample
  ...
  beta1 <- -4.6 #Our choice for the slope. 
  
  beta2 <- -2
  
  beta3 <- 1.03

  sigma <- .75 #Our choice for the std. deviation of the error terms.
  ...
  #Fit an estimated regression model to the fabData.
  fab.lm <- lm(y ~ x + I(x^2) + I(x^3), data=fabData) 
  
  summary(fab.lm) #Summarize your model. 
  b <- coef(fab.lm)
  plot(y ~ x, data=fabData, pch=20, col="skyblue") #Plot the data.

  #abline(fab.lm) #Add the estimated regression line to your plot.
  curve(b[1] + b[2]*x + b[3]*x^2 + b[4]*x^3, add=TRUE, col="skyblue")

  # Now for something you can't do in real life... but since we created the data...

  #abline(beta0, beta1, lty=2)
  #Add the true regression line to your plot using a dashed line (lty=2).  
  curve(beta0 + beta1*x + beta2*x^2 +  beta3*x^3, add=TRUE, col="skyblue", lty=2)

  legend("top", legend=c("True Cubic", "Estimated Cubic"), lty=c(2,1), bty="n",
         col="skyblue", cex=0.5) 
...
#  ## Simulating Data from a Regression Model
  ...
  X_i <- runif(n, 30, 50) 
    #Gives n random values from a uniform distribution between 15 to 45.
  X_2i <- sample(c(0,1), n, replace=TRUE) #on/off switch

  beta0 <- -10   #Our choice for the y-intercept. 

  beta1 <-  1.32 #Our choice for the slope. 
  
  beta2 <-  71.5 #Change in intercept
  
  beta3 <- -1.57 #Change in slope

  sigma <- 1.5 #Our choice for the std. deviation of the error terms.
  ...
  plot(y ~ x, data=fabData, col=colors[as.factor(x2)], pch=20) #Plot the data.

  #Add the estimated regression lines.
  x2 = 0
  curve(b[1] + b[2]*x + b[3]*x2 + b[4]*x*x2, add=TRUE, col=colors[1])
  x2 = 1
  curve(b[1] + b[2]*x + b[3]*x2 + b[4]*x*x2, add=TRUE, col=colors[2])
  ...
  #Add the true regression line to your plot using a dashed line (lty=2). 
  x2=0
  curve(beta0 + beta1*x + beta2*x2 +beta3*x*x2, add=TRUE, col=colors[1], lty=2)
  x2=1
  curve(beta0 + beta1*x + beta2*x2 +beta3*x*x2, add=TRUE, col=colors[2], lty=2)
  
  #Add a legend to your plot specifying which line is which.
  legend("top", legend=c("True Blue Line", "True Orange Line",
                        "Estimated Orange Line", "Esimated Orange Line"), lty=c(2,2,1,1),
        col=colors, bty="n", cex=0.5) 
```
