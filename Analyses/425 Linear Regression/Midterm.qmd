...
?? The key term is actual looking for an actual value. He said he can't figure out how to ask for confidence interval without using the word average
...
The confidence interval for intercept and slope calculated the same, but the calc for sd is different
Intercept sd is the variability of the MSE plus the slope because the farther out the x the more variability in Y
Slope MSE vertical variability over spread for slope (rise over run)
The tvalue is the number of standard errors the sample is from the truth? pvalue is the probability we'll have a sample as extreme or more extreme if the hyp is true.We know samples can vary. We use our sample's variability to estimate the deviation in general. The pvalue tells us how surprisinng our samples is if the null hypothesis is true.