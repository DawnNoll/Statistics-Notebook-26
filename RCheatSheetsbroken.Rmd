...
### Assessment
#A regresion is performed and the following plot created. Images a residuals v fitted plot. Which of the following correctly shows the original scatterplot of this regression?
#resdiual is the obs-fit, so the r=act-fit so fit+residual=actual
```
...
# Assessment
#2Below is the summary output from a simple linear regression performed in R. Compute the 95% confidence interval for the slope estimate.
Ans: (7.701, 9.423)
...
# which has the best linearity (which line is straightest, 
# not which closest to dotted line )
a <- lm(height~age + I(age^2), data=Loblolly)
b <- lm(height~age+ I(age^2) + I(age^3) +I(age^4), data=Loblolly)
c <- lm(height~age+ I(age^2) + I(age^3) +I(age^4) + I(age^5), data=Loblolly)
d <- lm(height~age+ I(age^2) + I(age^3), data=Loblolly)
par(mfrow = c(2, 2))# look at more than one grid
plot(a, which=1, main="A")
plot(b, which=1, main="B")
plot(c, which=1, main="C")
plot(d, which=1, main="D")
...
For the graphs to the left and right of the columnname in the pairs plot, that column is the Y axis, and the columnname below the graph is the x axis

##### Quizzes


```{r, eval=FALSE, fig.height=10, fig.width=10}
# All of problem one's sections are to compare the adjR2 to find the best fit
# Fit this model to the KidsFeet dataset. Use your model to answer the questions and re-create the graph shown below.
library(mosaic)
View(KidsFeet)

#Y_i(length) = B0 + B1*X_i1 + B2*X_i1^2 + epsilon (x_i1 is width)
# quadratic
kfquad.lm <- lm(length ~ width + I(width^2), data=KidsFeet)
summary(kfquad.lm) #remember look at adjR2
b <- coef(kfquad.lm)
...
# #Y_i(length) = B0 + B1*X_i1 + B2*X_i1^2 + epsilon (x_i1=width, x_i2=sex)
# 2 lines, width and sex 
...
ggplot(KidsFeet, aes(y=length, x=width, color=sex)) +
  ...
  stat_function(fun=function(x) (b[1] + b[3]) + b[2]*x, lty=1, aes(color="G")) + #notice 1+3 is second intercept, 2 is slope since no 4
  scale_colour_manual("sex", values = c("skyblue", "red", "skyblue", "red")) +
  labs(title="KidsFeet dataset") +
  theme(legend.justification = c(0,1), legend.position = c(0,1)) +
  theme_minimal()

# # #Y_i(length) = B0 + B1*X_i1 + B2*X_i1^2 + B3*X_i2 + B4*X_i1^2*X_i2 + epsilon (x_i1=width, x_i2=sex)
# 2 quadratics
...
ggplot(KidsFeet, aes(y=length, x=width, color=sex)) +
  ...
  stat_function(fun=function(x) (b[1] + b[4]) + b[2]*x + (b[3] + b[5])*(x^2), lty=1, aes(color="G")) + # this one is missing the x term for the second equation so it's just the first x term without adding a second x term
  scale_colour_manual("sex", values = c("skyblue", "red", "skyblue", "red")) +
  labs(title="KidsFeet dataset") +
  theme(legend.justification = c(0,1), legend.position = c(0,1)) +
  theme_minimal()
...
#3
#Select (by clicking on it) the row of the pairs plot below that corresponds to `elecbill` being the y-variable in each of the plots. This is the main row you want to look at as you decide which variable is most useful in explaining y, the elecbill. For pairs it says elec bill
# Which variable is "visibly" the "best" according to the pairs plot for predicting elecbill?  Click on the correct plot and name the column. kwh the dots line up with the red line best
library(pander)
?Utilities
pairs(Utilities, panel=panel.smooth)

ut.lm <- lm(elecbill ~ kwh, data=Utilities)
summary(ut.lm)
...
# Create a new pairs plot that includes the residuals and fitted values from the previous regression. Hint: `cbind(R = lm1$res, Fit = lm1$fit, Utilities
pairs(cbind(R=ut.lm$res, Fit=ut.lm$fit, Utilities), panel=panel.smooth)
#Thus, while many variables could give some insight on the residuals, month seems to explain most of those variables as well, so that would suggest using month as a variable that could explain elecbill. Also, month would be the best choice to actually use over any of the other related variables, because it would be the easiest variable to "measure" in real life. However, there is one other variable that does not relate to month that also seems to shed some insight on the residuals from the regression from Part (b). Ans: year
# adding in other useful vars
ut3.lm <- lm(elecbill~kwh+month+year,data=Utilities)
summary(ut3.lm)
pander(summary(ut3.lm))
plot(ut3.lm, which=1)

# add in quadratic because that's what month looks like
# Y_i = B0 + B1X1 + B2X2 + B3X3 + B4X2^2 + epsilon
ut3quad.lm <- lm(elecbill~kwh + month + year + I(month^2), data=Utilities)
summary(ut3quad.lm)
plot(ut3quad.lm, which=1)
...
pairs(cbind(Res=ut3quad.lm$res, Fit=ut3quad.lm$fit, Utilities), panel=panel.smooth)
# new lm using res vs month year temp
ut4quad.lm <- lm(elecbill ~ kwh + month + year + I(month^2) + temp + I(temp^2), data=Utilities)
summary(ut4quad.lm)

# drop month and I month since not sig
ut3quadnew.lm <- lm(elecbill ~ kwh + year + temp + I(temp^2), data=Utilities)
summary(ut3quadnew.lm)
plot(ut3quadnew.lm, which=1)
...
Ut <- Utilities[-95,]
View(Ut)
boxCox(ut3quadnew.lm) # lambda from .5 to 1, so we'll leave it

# re-run without obs 95
ut3qnew.lm <- lm(elecbill ~ kwh + year + temp + I(temp^2), data=Ut)

summary(ut3qnew.lm)
pander(summary(ut3qnew.lm))
...
#### why bind res and fit to data for pairs plot?
# According to google Combining the residuals and fitted values (often along with predictors) into a pairs plot—or a scatterplot matrix—is helpful because it allows you to explicitly link modeling errors back to specific predictor variables and their ranges. 
# When the Residuals v Fitted plot shows a poor fit (e.g., non-random, curved patterns or fanning), it tells you the model is broken, but not why. Looking at the pairs plot of (data, residuals, fitted) provides a direct visual link between the error and the input variables, making it easier to diagnose the specific issue. 
# Here is why this method is valuable:
# Identifies Hidden Nonlinearity: You can see if a pattern in the residuals (e.g., a "U" shape) matches a nonlinear trend in a specific predictor (
# ), indicating a missing quadratic term (
# ).
# Diagnoses Heteroscedasticity: It helps you identify which predictor corresponds to widening residuals, showing you which variable causes non-constant variance (e.g., variance increases as 
#  increases).
# Links Residuals to Data Points: In multiple regression, the fitted values are a combination of predictors. The pairs plot lets you trace a single, high-residual point across all predictors to see if it is a leverage point, outlier, or simply incorrectly modeled.
# Shows Interaction Effects: You may see that the residuals show a pattern specifically when two predictors, 
#  and 
# , are both high, suggesting an interaction term (
# 
# ) is needed. 

# left her because has the color interaction code
pairs(cbind(Res=ut.lm$res,Fit=ut.lm$fit,rbdata_adv), panel=panel.smooth)
